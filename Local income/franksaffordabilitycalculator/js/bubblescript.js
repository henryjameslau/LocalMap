		...
		function drawGraphic()
		{
			...
			for (var column in graphic_data[0]) {
				...
				dots = graphic_data.map(function(d,i) {
					...
					// return obecjt variables to use in code. This needs to be updated in accordance with field content of files
					return {
						'group' : d.group,
						'name' : d.name,
						'0' : +d["percentile"],
			...
			if ( graphic.width() > dvc.config.optional.mobileBreakpoint ) {
				...
				//if ( isFirefox == false ) {
					...
					$(element).popover({
							...
							trigger: 'manual',
							html : true,							
							content: function() { return "<span style='font-size: 14px; text-align: center;'><b>" + d.name + "</b></span><br><span style='font-size: 14px; text-align: center;'><b>Weekly income: £" + numberWithCommas(d[dvc.selectedYVariableIndex]) + "</b></span>"; }
					});
...
//				//	$("#toolTip").style("position" , "absolute").style("top" , dvc.y1 + "px").style("left" , dvc.x1 + "px"); // throws error ...
		...
		*/
		...
		if (Modernizr.inlinesvg)
		{
			...
			d3.json("data/configbubble.json", function(error, json)
			{	
