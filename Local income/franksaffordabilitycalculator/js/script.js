...
//╱╱╱╱╱╱╱╱╰╯╱╰╯╱╱╱╱╱╰━━╯
	...
	function ready (error, topoLSOA, topoLAD, boundsData, config){
					...
					if($(window).width() >= 650){
						...
						map = L.map('map',{maxZoom:13,minZoom:11}),
...
function addRemoveAreas(oldDistricts, newDistricts) {
	...
	function buildDataInView(d,csvData) {
			...
			flattenedData.forEach(function(d) {rateById[d.MSOA_code] = +eval("d.Total_weekly_income"); 
			areaById[d.MSOA11CD] = d.MSOA11NM});
...
console.log(data)
		...
		rateById = {};
		flattenedData.forEach(function(d) { rateById[d.MSOA11CD] = +eval("d." + dvc.curr); });
