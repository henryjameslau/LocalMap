	...
	queue()
		.defer(d3.json, "assets/Middle_Layer_Super_Output_Areas_December_2011_Super_Generalised_Clipped_Boundaries_in_England_and_Wales.topojson")
...
// Create an object to give yourself a pair of values for the parlicon code and data value
	...
	rateById = {};
	data.forEach(function(d) { rateById[d.msoa11cd] = +eval("d." + dvc.curr); });
...
// Then append a path element for every bit of data you've just binded.
	...
	zoomed();
		  ...
		  g.attr("class", "geogg")
		  .selectAll("path")
		  .data(topojson.feature(geog, geog.objects.collection).features)
		  .enter()
		  .append("path")
		  .attr("id",function(d){return "reg" + d.properties.msoa11cd})
		  .attr("data-nm", function(d){return d.properties.msoa11nm})
		  .attr("data-dt", function(d) {return rateById[d.properties.msoa11cd];})
		  .attr("d", path)
		  .style("fill", function(d) {if (typeof(color(rateById[d.properties.msoa11cd])) != "undefined") { return color(rateById[d.properties.msoa11cd]); }else {return "#e0e0e0"}})
		  .on("mouseout",unhighlight)
		  .on("mouseover",function(d){highlight(d.properties.msoa11cd)});
